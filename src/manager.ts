...
const oauth = (credential: OAuthCredentials): OAuthCredential => ({
	type: "oauth",
	...credential,
});
const sameCredential = (
	left: OAuthCredential | undefined,
	right: OAuthCredential | undefined,
) =>
	left !== undefined &&
	right !== undefined &&
	JSON.stringify(left) === JSON.stringify(right);
const isPermanent = (error: unknown) =>
	/invalid_grant|unauthorized|forbidden|\b40[013]\b/i.test(
...
export class ProfileManager {
	...
	supportedProviders(): string[] {
		return [...this.providers.keys()].sort((left, right) =>
			left.localeCompare(right),
		);
	}
	models(provider: string): string[] {
		return this.pi.models(provider);
	}
	...
	async bootstrap(): Promise<void> {
		...
		for (const provider of this.supportedProviders()) {
			...
			if (current?.type === "oauth" && !state.providers[provider])
				await this.vault.add(provider, "Default", current);
		}
	}
	...
	async capture(
		...
		if (requested) {
			await this.vault.add(
				provider,
				this.validateName(requested),
				oauth(credentials),
			);
			return requested;
		}
		return (await this.vault.addUnique(provider, "Profile", oauth(credentials)))
			.name;
	}
	...
	async finalizeLogin(
		...
		await this.vault.change((state) => {
			...
			if (
				scoped?.profiles[name] &&
				sameCredential(this.pi.getCredential(provider), credential)
			)
	...
	async activate(provider: string, name: string, model: string): Promise<void> {
		this.requireProvider(provider);
		if (!this.pi.models(provider).includes(model))
			throw new Error("Selected model is not compatible with this provider");
		try {
			await this.vault.transaction(
				...
				async (result) => {
					result.beforeCredential
						? this.pi.setCredential(provider, result.beforeCredential)
	...
	async rename(
		...
		await this.vault.change((state) => {
			...
			if (!profile) throw new Error("Profile not found");
			if (scoped.profiles[to])
				throw new Error(`Profile "${to}" already exists for ${provider}`);
	...
	async delete(
		provider: string,
		name: string,
	): Promise<"deleted" | "switched" | "detached"> {
		try {
			const outcome = await this.vault.transaction(
				(state) => {
					...
					const replacement = Object.keys(scoped.profiles).find(
					...
					if (active && replacement) {
						model =
							scoped.model && this.pi.models(provider).includes(scoped.model)
								? scoped.model
								: this.pi.models(provider)[0];
						credential = scoped.profiles[replacement]?.credential;
						if (!model || !credential)
							throw new Error("No compatible replacement is available");
						scoped.defaultProfile = replacement;
						result = "switched";
					} else if (active) result = "detached";
					delete scoped.profiles[name];
					if (!Object.keys(scoped.profiles).length)
				...
				async (value) => {
					if (value.result === "switched") {
						if (!value.credential || !value.model)
							throw new Error("Replacement profile is incomplete");
						this.pi.setCredential(provider, value.credential);
						if (!(await this.pi.setModel(value.model)))
							throw new Error("model selection failed");
					} else if (value.result === "detached")
						this.pi.removeCredential(provider);
				},
				async (value) => {
					value.beforeCredential
						? this.pi.setCredential(provider, value.beforeCredential)
						: this.pi.removeCredential(provider);
		...
		if (action === "restore") {
			await this.vault.transaction(
				(state) => {
					const scoped = state.providers[provider];
					const selected =
						scoped?.defaultProfile && scoped.profiles[scoped.defaultProfile];
			...
			await this.vault.change((state) => {
				...
				if (!mirror || !selected)
					throw new Error("A mirror and Provider Default are required");
	...
	async refreshAll(onlyDue = false): Promise<void> {
		...
		for (const [providerId, scoped] of Object.entries(state.providers)) {
			...
			for (const profile of Object.values(scoped.profiles)) {
				if (
					onlyDue &&
					(profile.credential.expires > this.now() + 300_000 ||
						(profile.nextRetryAt ?? 0) > this.now())
				)
					continue;
	...
	async status(provider: string): Promise<string> {
		...
		if (!sameCredential(this.pi.getCredential(provider), profile.credential))
			return `${provider}/${name} (detached)`;
			...
			await this.vault.transaction(
				(state) => {
					const current = Object.values(
						state.providers[provider.id]?.profiles ?? {},
				...
				(result) => {
					if (
						...
						result.active &&
						sameCredential(
							this.pi.getCredential(provider.id),
							result.beforeCredential,
						)
					) {
				...
				(result) => {
					...
					result.beforeCredential
						? this.pi.setCredential(provider.id, result.beforeCredential)
						: this.pi.removeCredential(provider.id);
				},
			);
		} catch (error) {
			await this.vault.change((state) => {
				...
				if (isPermanent(error)) current.status = "needs-login";
					...
					current.nextRetryAt =
						this.now() +
						([30_000, 120_000, 600_000][current.retryCount - 1] ?? 600_000);
				}
			});
		}
	}
	...
	private validateName(name: string): string {
		if (!name || name.length > 64 || /[\\/\0]/.test(name))
			throw new Error("Profile names must be 1-64 characters without slashes");
		return name;
	}
}
