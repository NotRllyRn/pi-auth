...
export class Vault {
	...
	async add(
		...
		return this.change((state) =>
			clone(this.insert(state, provider, name, credential)),
		);
	}
	...
	async addResolved(
		...
		return this.change((state) =>
			clone(this.insert(state, provider, name, resolve())),
		);
	}
	...
	async addUnique(
		...
		base: string,
		credential: OAuthCredential,
	): Promise<{ name: string; profile: CredentialProfile }> {
		return this.change((state) => {
			...
			return {
				name,
				profile: clone(this.insert(state, provider, name, credential)),
			};
		});
	}
	...
	private insert(
		...
		if (scoped.profiles[name])
			throw new Error(`Profile "${name}" already exists for ${provider}`);
	...
	private findById(
		...
		const profile = Object.values(
			state.providers[provider]?.profiles ?? {},
	...
	private async load(): Promise<VaultState> {
		try {
			...
			if ((error as NodeJS.ErrnoException).code === "ENOENT")
				return emptyVault();
			await rename(this.path, `${this.path}.corrupt`).catch(
				async (renameError) => {
