...
function setup() {
	...
	const provider: OAuthProviderInterface = {
		...
		refreshToken: async (value) => credential(`${value.access}-fresh`),
	...
	return {
		...
		runtime,
		adapter: {
			getCredential: (id: string) => mirror.get(id),
			setCredential: (
				id: string,
				value: { type: "oauth" } & OAuthCredentials,
			) => {
				mirror.set(id, value);
			},
			...
			setModel: async (value: string) =>
				runtime.failModel ? false : ((runtime.model = value), true),
			models: (id: string) => [`${id}/new`],
		},
	};
}
...
async function manager() {
	...
	return {
		...
		manager: new ProfileManager(
			new Vault(join(dir, "vault.json")),
			env.adapter,
			new Map([[env.provider.id, env.provider]]),
		),
	};
}
...
test("activation updates mirror, model, default, and status", async () => {
	...
	assert.equal(
		(await env.manager.state()).providers.anthropic?.defaultProfile,
		"Work",
	);
...
test("failed activation restores the prior mirror and model", async () => {
	...
	await assert.rejects(
		env.manager.activate("anthropic", "Work", "anthropic/new"),
		/previous selection was restored/,
	);
...
test("rename, deletion, and reconciliation preserve explicit outcomes", async () => {
	...
	assert.equal(
		(await env.manager.state()).providers.anthropic?.profiles.One?.credential
			.access,
		"external",
	);
...
test("rejects a stale refresh result", async () => {
	const env = await manager();
	const profileName = await env.manager.capture(
		"anthropic",
		credential("old", 0),
		"Work",
	);
	...
	env.provider.refreshToken = async () =>
		new Promise((resolve) => {
	...
	const profile = (await env.manager.state()).providers.anthropic?.profiles[
		profileName
	];
	assert.ok(profile);
	await env.manager.vault.updateCredential("anthropic", profile.id, {
		type: "oauth",
		...credential("new"),
	});
	...
	await refreshing;
	assert.equal(
		(await env.manager.state()).providers.anthropic?.profiles.Work?.credential
			.access,
		"new",
	);
});
...
test("inactive refresh cannot overwrite a concurrent Activation", async () => {
	...
	env.provider.refreshToken = async (value) =>
		value.access === "work"
			? new Promise((resolve) => {
					resolveRefresh = resolve;
					started();
...
test("refresh retains rotation and marks permanent failures without deletion", async () => {
	...
	await env.manager.refreshAll();
	assert.equal(
		(await env.manager.state()).providers.anthropic?.profiles.Work?.status,
		"needs-login",
	);
});
