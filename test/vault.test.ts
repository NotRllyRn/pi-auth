...
test("stores provider-scoped profiles and defaults with private permissions", async () => {
	...
	assert.equal(
		state.providers["openai-codex"]?.profiles.Default?.credential.access,
		"b",
	);
...
test("rejects duplicate names and stale refresh writes", async () => {
	...
	await assert.rejects(
		vault.add("anthropic", "Default", credential("other")),
		/already exists/,
	);
	await vault.updateCredential("anthropic", profile.id, credential("new"));
	assert.equal(
		await vault.commitRefresh(
			"anthropic",
			profile.id,
			profile.generation,
			credential("stale"),
	...
	assert.equal(
		(await vault.read()).providers.anthropic?.profiles.Default?.credential
			.access,
		"new",
	);
});
...
test("serializes concurrent profile changes", async () => {
	...
	assert.equal(
		Object.keys(
			(await new Vault(path).read()).providers.anthropic?.profiles ?? {},
		).length,
		12,
	);
});
	...
	const run = (prefix: string) =>
		new Promise<void>((resolve, reject) => {
			...
			child.once("exit", (code) =>
				code === 0
					? resolve()
					: reject(new Error(`lock worker exited ${code}`)),
			);
		});
